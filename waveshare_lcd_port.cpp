...
void waveshare_lcd_init(void)
{
    ...
    lcd->reset();
    assert(lcd->begin());
    if (lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(esp_panel::drivers::LCD::BasicBusSpecification::FUNC_DISPLAY_ON_OFF)) {
        lcd->setDisplayOnOff(true);
    }

    Serial.println("LCD initialized successfully");
    /* Comment out color bar test
...
// Function to initialize the LCD and return pointer
esp_panel::drivers::LCD* waveshare_lcd_init_and_get(void)
{
